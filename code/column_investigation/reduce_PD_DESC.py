...
for line in sys.stdin:
    ...
    #    continue
    #Get key/value 
    PD_DESC, value = line.split('\t')
    dtype, stype, label = value.split(',')

        
    if PD_DESC in PD_DESC_Dict:
        PD_DESC_Dict[PD_DESC] = PD_DESC_Dict[PD_DESC] + 1
    else:
        PD_DESC_Dict[PD_DESC] = 1
...
for key in sorted(PD_DESC_Dict):
    print ('%s\t%d,%s' %(key, PD_DESC_Dict[key], 'value'))
